...
# weitere Scripte und nimmt Anpassungen an Diensten und Configdateien vor und startet den Client insgesamt         #
# zweimal neu.                                                                                                     #
# Nach dem zweiten Reboot läuft das System direkt in die Anmeldung zur Hipos VKS Seite. Es identifiziert sich      #
# als Telefon und sendet DSCP 34 tags (dezimal 34 ~ AF41). Durch die dynamische Voice VLAN Auswahl kann so der     #
# traffic als Videotelefonie priorisiert werden. 																   #
...
#      Preface root für pulse Audio berechtigen um Hardware sauber auslesen zu können

cat <<EOF | tee /etc/systemd/system/pulseaudio.service
[Unit]
...
/usr/sbin/adduser root pulse-access
...
LAST=""
while true; do
IF=\$(ip -4 route ls default | grep -Po '(?<=dev )(\S+)' | head -n 1)
for i in \$(seq 1 11)
	do
