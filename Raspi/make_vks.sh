...
apt install net-tools xdotool lldpd snmpd curl gnupg ca-certificates original-awk nftables isc-dhcp-client rsync chromium python3 python3-tk gnome-keyring -y

LOGINUSER=$(ls /home | grep -v root)
mkdir -p /scripts
mkdir -p /overlay
...
cp ./overlay.py /scripts


cat <<EOF | tee /scripts/start.sh
#!/bin/bash
xset s off &
xset s noblank &
xset -dpms &
sleep 5
/usr/bin/chromium --kiosk "https://join.hipos-vks.polizei.nrw" --incognito &
sleep 5
TOKEN=0
...
chmod 750 /scripts/start.sh
chown root:vksuser /scripts/start.sh
...
echo ""DAEMON_ARGS="-c -C eno1 -M 3">> /etc/default/lldpd
...
for i in \$(seq 1 11)
	do
		...
		if [ "\$VOICE" != "\$LAST" ] && [ -n "\$VOICE" ]; then
			...
			# ja dann halt nicht ...
			ip route | grep default && ip route del default
			ip link delete \$IF.\$LAST 2>/dev/null
...
chown -R \$LOGINUSER:\$LOGINUSER "\$DST"
EOF
...
[Unit]
Description=Reset home directory for \$LOGINUSER
...
cat <<EOF | tee /home/vksuser/.config/autostart/chromium.desktop
[Desktop Entry]
Type=Application
Name=Vivaldi
Hidden=false
Exec=/bin/sh /scripts/start.sh
...
cat <<EOF | tee /home/vksuser/.config/autostart/set_run.desktop
[Desktop Entry]
Encoding=UTF-8
Version=0.9.4
Type=Application
Name=Vivaldi_Preference
Hidden=false
Exec=/scripts/set_run.sh
OnlyShowIn=XFCE;
RunHook=0
StartupNotify=false
Terminal=false
...
curl -sL "https://github.com/azlux/log2ram/archive/refs/tags/${LOG2RAM_VER}.tar.gz" -o "log2ram.tar.gz"
echo "${LOG2RAM_SHA256}  log2ram.tar.gz" | sha256sum -c - || exit 1

tar zxf log2ram.tar.gz
cd "log2ram-${LOG2RAM_VER}"
chmod +x install.sh && ./install.sh
cd ..
rm -r "log2ram-${LOG2RAM_VER}"
rm log2ram.tar.gz
...
cp ./boot/firmware/overlay.py /scripts/
cp ./make_vks.sh /scripts
...
systemctl disable make_vks.service
rm -f /etc/systemd/system/make_vks.service
